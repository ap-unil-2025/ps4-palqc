...
def backup_file(source_filename, backup_filename):
    ...
    try:
        ...
        save_contacts_to_file(data, backup_filename)
        return True
    except Exception:
        return False
...
def get_file_stats(filename):
    ...
    try:
        ...
        return {'exists': os.path.exists(filename), 'type': type(data).__name__, 'count': len(data), 'size_bytes': os.path.getsize(filename)}
    except Exception:
        return None
...
def merge_json_files(file1, file2, output_file):
    ...
    try:
        if os.path.exists(file1) and os.path.exists(file2):
            ...
            if type(data1).__name__== 'list' and type(data1).__name__==type(data2).__name__:
                data_combined=data1 + data2
                save_to_json(data_combined, output_file)
                return True
    except Exception:
        return False
